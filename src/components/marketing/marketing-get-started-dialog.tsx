    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Set up OpenHire locally or deploy to production.</DialogDescription>
        </DialogHeader>
        <Tabs defaultValue={GET_STARTED_TABS[0]?.id ?? "run-locally"} className="w-full">
          <TabsList>
            {GET_STARTED_TABS.map((tab) => {
              const Icon = iconByType[tab.icon];
